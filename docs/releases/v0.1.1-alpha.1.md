...
The included feature batches passed their recorded owner-operated AYN Thor checks in Debug builds, including Profile migration, Guide reading, circular magnifier behavior, Enhanced Touch protection, controller navigation replay, loading stability, Macro icon-only presentation, Canvas circular crop, and touch-surface cancellation. Build and static verification do not replace the final check of the exact real-signed `0.1.1-alpha.1` candidate; that candidate must still complete the release acceptance matrix before publication.
...
本版本包含的功能批次均通过了已记录的 AYN Thor 机主 Debug 实机检查，覆盖 Profile 迁移、Guide 阅读、圆形放大镜、增强触控保护、系统导航手柄宏、游戏加载稳定性、Macro 仅图标、Canvas 圆形裁剪和触控面退出。Build 与静态检查不能替代最终真实签名 `0.1.1-alpha.1` candidate 的实机验收；发布前仍必须对该 exact candidate 完成 release 验收矩阵。
