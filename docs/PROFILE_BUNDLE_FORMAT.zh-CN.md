...
导出器会收集受支持的 Profile 图标、地图、文件 Guide、用户导入的 Macro 图标和 Canvas 媒体。内置 Macro 图标与在线链接继续作为普通结构化引用保存。任何必需资源缺失、不可读、不受支持或超出大小限制时，整个导出都会失败，不会产生部分可用的迁移包。
...
Schema 版本 1 将单个资源限制为 128 MB，未压缩资源总量限制为 512 MB，迁移包输入限制为 512 MB，资源数量限制为 512，Profile JSON 限制为 16 MB，清单限制为 1 MB。可存储扩展名包括 JPG、PNG、WebP、GIF、MP4、PDF、TXT、Markdown 和 HTML；每个引用字段仍可能应用更严格的规则。Canvas 媒体另有 50 MiB 上限；图片不超过 4096 × 4096，MP4 视频最长边不超过 2048。
